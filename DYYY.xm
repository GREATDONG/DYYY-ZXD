...
#pragma mark - DYYY IM Chat Enhancement Hooks
    ...
    if (accountServiceClass) {
        ...
        if (class_getClassMethod(accountServiceClass, sharedSel)) {
            ...
            if (service) {
                ...
                SEL currentUserSel = NSSelectorFromString(@"currentUser");
                if ([service respondsToSelector:currentUserSel]) {
                    id currentUser = ((id (*)(id, SEL))objc_msgSend)(service, currentUserSel);
                    if (currentUser) {
                        SEL uidSel = NSSelectorFromString(@"userId");
                        if ([currentUser respondsToSelector:uidSel]) {
                            return ((NSString *(*)(id, SEL))objc_msgSend)(currentUser, uidSel);
                        }
                    }
                }
            }
...
static void DYYYQuoteMessage(id cell, id message) {
    ...
    // 尝试通过 ChatVC 触发引用
    // 方法1: 查找聊天 VC 并调用 quoteMessage
    UIResponder *responder = (UIResponder *)cell;
    while (responder) {
        SEL quoteSel = NSSelectorFromString(@"quoteMessage:");
        if ([responder respondsToSelector:quoteSel]) {
            ((void (*)(id, SEL, id))objc_msgSend)(responder, quoteSel, message);
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"DYYYQuoteMessageNotification"
                                                        object:nil
                                                      userInfo:@{@"message": message ?: [NSNull null]}];
    
    // 方法3: 模拟菜单中的引用按钮点击
    Class menuCompClass = objc_getClass("AWEIMCustomMenuComponent");
    if (menuCompClass) {
        // 找到引用菜单项并触发
        SEL quoteActionSel = NSSelectorFromString(@"msg_didSelectQuoteMessage:");
        if (cell respondsToSelector:quoteActionSel) {
            ((void (*)(id, SEL, id))objc_msgSend)(cell, quoteActionSel, message);
            [DYYYToast showSuccessToastWithMessage:@"已引用消息"];
            return;
        }
    }
...
static void DYYYRecallMessage(id cell, id message) {
    ...
    NSString *currentID = DYYYGetCurrentUserID();
    if (!senderID || !currentID || ![senderID isEqualToString:currentID]) {
        [DYYYToast showSuccessToastWithMessage:@"只能撤回自己的消息"];
        return;
    }
    
    // 方法1: AWERecallMessageManager
    Class recallMgrClass = objc_getClass("AWERecallMessageManager");
    if (recallMgrClass) {
        SEL sharedSel = NSSelectorFromString(@"sharedInstance");
        if (!sharedSel) sharedSel = NSSelectorFromString(@"shared");
        if (class_getClassMethod(recallMgrClass, sharedSel)) {
            id recallMgr = ((id (*)(id, SEL))objc_msgSend)(recallMgrClass, sharedSel);
            if (recallMgr) {
                SEL recallSel = NSSelectorFromString(@"recallMessage:");
                if ([recallMgr respondsToSelector:recallSel]) {
                    ((void (*)(id, SEL, id))objc_msgSend)(recallMgr, recallSel, message);
    ...
    // 方法2: IESIMRecallMessageTextService
    Class recallSvcClass = objc_getClass("IESIMRecallMessageTextService");
    if (recallSvcClass) {
        SEL recallSel = NSSelectorFromString(@"recallMessage:completion:");
        id recallSvc = [[recallSvcClass alloc] init];
        if (recallSvc && [recallSvc respondsToSelector:recallSel]) {
            ((void (*)(id, SEL, id, id))objc_msgSend)(recallSvc, recallSel, message, ^(id result) {
                [DYYYToast showSuccessToastWithMessage:@"已撤回消息"];
            });
            return;
        }
    ...
    // 方法3: 通过 responder chain 找到聊天 VC
    UIResponder *responder = (UIResponder *)cell;
    while (responder) {
        SEL recallSel = NSSelectorFromString(@"recallMessage:");
        if ([responder respondsToSelector:recallSel]) {
            ((void (*)(id, SEL, id))objc_msgSend)(responder, recallSel, message);
            [DYYYToast showSuccessToastWithMessage:@"已撤回消息"];
            return;
        }
        responder = [responder nextResponder];
...
- (void)didEnterVisitorsPage {
    if (DYYYGetBool(@"DYYYBlockVisitorUpload")) {
        NSLog(@"[DYYY] Blocked visitors page enter report");
        return;
    }
    %orig;
}
...
// 策略2: Hook NSURLSession HTTP 方法，精确匹配抖音访客上报 API
// 抖音访客上报 URL 通常包含 /user/profile/visit 或 /visitor/report
%hook NSURLSession
    ...
    if (DYYYGetBool(@"DYYYBlockVisitorUpload")) {
        ...
        if (url) {
            ...
            BOOL isAwemeDomain = ([host containsString:@"aweme"] || 
                                  ...
                                  [host containsString:@"tiktok"] ||
                                  [host containsString:@"bytedance"]);
            BOOL isVisitorAPI = ([path containsString:@"/visitor/"] ||
                                 [path containsString:@"/profile/visit"] ||
                                 [path containsString:@"/user/visit"] ||
                                 [path containsString:@"/visit/log"] ||
                                 [path containsString:@"/homepage/visit"]);
            if (isAwemeDomain && isVisitorAPI) {
                NSLog(@"[DYYY] Blocked visitor upload: %@%@", host, path);
                // 返回空响应而非取消请求（避免上层报错）
                if (completionHandler) {
                    NSDictionary *emptyDict = @{};
                    NSData *emptyData = [NSJSONSerialization dataWithJSONObject:emptyDict options:0 error:nil];
                    NSHTTPURLResponse *emptyResponse = [[NSHTTPURLResponse alloc]
                                                        initWithURL:url
                                                        statusCode:200
...
%ctor {
    ...
    if (DYYYGetBool(@"DYYYEnableSwipeActions")) {
        Class imCellClass = objc_getClass("AWEIMReusableCommonCell");
        if (imCellClass) {
